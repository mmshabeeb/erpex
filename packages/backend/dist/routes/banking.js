...
bankingRoutes.get('/statements', async (req, res, next) => {
    try {
        ...
        const statements = await bankingService.listBankStatements(companyId, req.query.accountId);
...
bankingRoutes.post('/statements/upload', upload.single('file'), async (req, res, next) => {
    try {
        ...
        const companyId = req.companyId;
        const statement = await bankingService.createBankStatement(companyId, accountId, req.file.originalname, periodStart, periodEnd, lines);
...
bankingRoutes.get('/reconciliation/:accountId', async (req, res, next) => {
    try {
        const companyId = req.companyId;
        const view = await bankingService.getReconciliationView(companyId, req.params.accountId, req.query.statementId);
...
bankingRoutes.post('/reconciliation/match', async (req, res, next) => {
    try {
        ...
        if (!matches || !Array.isArray(matches))
            throw new AppError('matches array required', 400);
