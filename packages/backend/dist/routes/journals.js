...
journalRoutes.get('/', async (req, res, next) => {
    try {
        const result = await journalService.listJournals(req.companyId, req.query);
...
journalRoutes.get('/:id', async (req, res, next) => {
    try {
        const journal = await journalService.getJournal(req.companyId, req.params.id);
...
journalRoutes.post('/', validateBody(createJournalSchema), async (req, res, next) => {
    try {
        const journal = await journalService.createJournal(req.companyId, req.body);
...
journalRoutes.patch('/:id/post', async (req, res, next) => {
    try {
        const journal = await journalService.postJournal(req.companyId, req.params.id);
        res.json({ success: true, data: journal });
    }
    catch (err) {
        next(err);
...
journalRoutes.post('/:id/rectify', async (req, res, next) => {
    try {
        const rectification = await journalService.createRectification(req.companyId, req.params.id, req.body.narration);
