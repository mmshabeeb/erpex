...
taxationRoutes.get('/configs', async (req, res, next) => {
    try {
        const configs = await taxService.listTaxConfigs(req.companyId);
...
taxationRoutes.post('/configs', validateBody(createTaxConfigSchema), async (req, res, next) => {
    try {
        const config = await taxService.createTaxConfig(req.companyId, req.body);
...
taxationRoutes.put('/configs/:id', validateBody(updateTaxConfigSchema), async (req, res, next) => {
    try {
        const config = await taxService.updateTaxConfig(req.companyId, req.params.id, req.body);
...
taxationRoutes.get('/report', async (req, res, next) => {
    try {
        ...
        const report = await taxService.getTaxReport(req.companyId, startDate, endDate);
...
taxationRoutes.get('/gstr1', async (req, res, next) => {
    try {
        ...
        const data = await generateGSTR1Data(req.companyId, new Date(startDate), new Date(endDate));
