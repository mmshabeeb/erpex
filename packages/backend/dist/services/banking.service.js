...
export async function createBankStatement(companyId, accountId, fileName, periodStart, periodEnd, lines) {
    ...
    if (!account.isCashOrBank)
        throw new AppError('Account is not a bank account', 400);
...
export async function getReconciliationView(companyId, accountId, statementId) {
    const account = await prisma.account.findFirst({
        where: { id: accountId, companyId },
    });
    if (!account)
    ...
    if (!statement)
        throw new AppError('No bank statement found for this account', 404);
    ...
    const reconciledSystemIds = new Set(statement.lines.filter(l => l.matchedItemId).map(l => l.matchedItemId));
...
function autoMatchTransactions(systemItems, bankLines) {
    const matches = [];
    ...
    for (const bankLine of bankLines) {
        ...
        for (const sysItem of systemItems) {
            if (matchedSystemIds.has(sysItem.id))
                continue;
            ...
            if (bankLine.reference && sysItem.journalEntry?.narration) {
                referenceMatch = bankLine.reference.toLowerCase().includes(sysItem.journalEntry.narration.toLowerCase().slice(0, 10)) || sysItem.journalEntry.narration.toLowerCase().includes(bankLine.reference.toLowerCase());
            }
...
// ─── Manual Clear ───────────────────────────────────────────
export async function clearStatementLines(lineIds) {
...
export async function listBankStatements(companyId, accountId) {
