...
function computeTotals(lines, discount = 0) {
    const subtotal = lines.reduce((s, l) => s + (l.qty * l.rate), 0);
    const taxTotal = lines.reduce((s, l) => s + (l.taxAmount || 0), 0);
    const total = subtotal + taxTotal - discount;
    return { subtotal, taxTotal, total };
}
export async function listEstimates(companyId, filters) {
...
export async function getEstimate(companyId, id) {
...
export async function createEstimate(companyId, data) {
...
export async function updateEstimateStatus(companyId, id, status) {
...
export async function convertToInvoice(companyId, estimateId) {
    ...
    if (!est)
        throw new AppError('Estimate not found', 404);
    if (est.status === 'INVOICED')
        throw new AppError('Estimate already invoiced', 400);
