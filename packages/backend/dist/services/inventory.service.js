 ...
 */
export async function recordInMovement(companyId, data) {
    const item = await prisma.item.findFirst({ where: { id: data.itemId, companyId } });
    if (!item || item.type !== 'PRODUCT')
        throw new AppError('Item is not a stockable product', 400);
 ...
 */
export async function recordOutMovement(companyId, data) {
    const item = await prisma.item.findFirst({ where: { id: data.itemId, companyId } });
    if (!item || item.type !== 'PRODUCT')
        throw new AppError('Item is not a stockable product', 400);
 ...
 */
export async function recordAdjustment(companyId, data) {
    const item = await prisma.item.findFirst({ where: { id: data.itemId, companyId } });
    if (!item || item.type !== 'PRODUCT')
        throw new AppError('Item is not a stockable product', 400);
    return prisma.inventoryMovement.create({
 ...
 */
export async function getItemMovements(companyId, itemId) {
    // Verify ownership
 ...
 */
export async function getLowStockAlerts(companyId) {
