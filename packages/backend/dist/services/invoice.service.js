 ...
 */
export async function postInvoice(companyId, id) {
    ...
    for (const line of inv.lines) {
        if (line.item && line.item.type === 'PRODUCT' && line.item.cogsAccountId && line.item.inventoryAccountId) {
            try {
                ...
                jeItems.push({ accountId: line.item.cogsAccountId, debit: cogs, credit: 0, narration: `COGS - ${line.item.name}` }, { accountId: line.item.inventoryAccountId, debit: 0, credit: cogs, narration: `Inventory out - ${line.item.name}` });
            }
            catch (e) {
...
export async function voidInvoice(companyId, id) {
    const inv = await prisma.invoice.findFirst({ where: { id, companyId } });
    if (!inv)
        throw new AppError('Invoice not found', 404);
    if (inv.amountPaid > 0)
        throw new AppError('Cannot void an invoice with payments', 400);
 ...
 */
export async function getUnpaidInvoices(companyId, contactId) {
