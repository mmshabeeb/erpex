...
purchaseOrderRoutes.get('/', async (req, res, next) => {
  try { res.json({ success: true, ...(await poService.listPurchaseOrders(req.query as any)) }); } catch (e) { next(e); }
});
purchaseOrderRoutes.get('/:id', async (req, res, next) => {
  try { res.json({ success: true, data: await poService.getPurchaseOrder(req.params.id) }); } catch (e) { next(e); }
});
purchaseOrderRoutes.post('/', async (req, res, next) => {
  try { res.status(201).json({ success: true, data: await poService.createPurchaseOrder(req.body) }); } catch (e) { next(e); }
});
purchaseOrderRoutes.patch('/:id/status', async (req, res, next) => {
  try { res.json({ success: true, data: await poService.updatePurchaseOrderStatus(req.params.id, req.body.status) }); } catch (e) { next(e); }
});
purchaseOrderRoutes.post('/:id/convert-bill', async (req, res, next) => {
  try { res.json({ success: true, data: await poService.convertToBill(req.params.id) }); } catch (e) { next(e); }
});

// ─── Bills ──────────────────────────────────────────────────

billRoutes.get('/', async (req, res, next) => {
  try { res.json({ success: true, ...(await billService.listBills(req.query as any)) }); } catch (e) { next(e); }
});
billRoutes.get('/unpaid/:contactId', async (req, res, next) => {
  try { res.json({ success: true, data: await billService.getUnpaidBills(req.params.contactId) }); } catch (e) { next(e); }
});
billRoutes.get('/:id', async (req, res, next) => {
  try { res.json({ success: true, data: await billService.getBill(req.params.id) }); } catch (e) { next(e); }
});
billRoutes.post('/', async (req, res, next) => {
  try { res.status(201).json({ success: true, data: await billService.createBill(req.body) }); } catch (e) { next(e); }
});
billRoutes.post('/:id/post', async (req, res, next) => {
  try { res.json({ success: true, data: await billService.postBill(req.params.id) }); } catch (e) { next(e); }
});

// ─── Expenses ───────────────────────────────────────────────

expenseRoutes.get('/', async (req, res, next) => {
  try { res.json({ success: true, ...(await expenseService.listExpenses(req.query as any)) }); } catch (e) { next(e); }
});
expenseRoutes.post('/', async (req, res, next) => {
  try { res.status(201).json({ success: true, data: await expenseService.createExpense(req.body) }); } catch (e) { next(e); }
});

// ─── Payments Made ──────────────────────────────────────────

paymentMadeRoutes.get('/', async (req, res, next) => {
  try { res.json({ success: true, ...(await paymentMadeService.listPaymentsMade(req.query as any)) }); } catch (e) { next(e); }
});
paymentMadeRoutes.post('/', async (req, res, next) => {
  try { res.status(201).json({ success: true, data: await paymentMadeService.createPaymentMade(req.body) }); } catch (e) { next(e); }
});
...
vendorCreditRoutes.post('/', async (req, res, next) => {
  try {
    const number = await generateDocNumber('VENDOR_CREDIT', 'vendorCredit');
