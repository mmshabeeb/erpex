...
export async function listContacts(filters: {
  type?: string; search?: string; page?: number; pageSize?: number;
}) {
  const where: any = {};
...
export async function updateContact(id: string, data: any) {
  ...
  if (addresses) {
    ...
    await prisma.contactAddress.createMany({
      data: addresses.map((a: any) => ({ ...a, contactId: id })),
    });
  }
...
export async function getContactStatement(id: string, startDate?: string, endDate?: string) {
