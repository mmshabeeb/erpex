...
export async function createFiscalYear(data: ICreateFiscalYearPayload) {
  ...
  if (overlap) throw new AppError('Fiscal year overlaps with existing year: ' + overlap.name, 409);

  // Generate monthly periods
  const periods: Array<{ month: number; year: number; startDate: Date; endDate: Date }> = [];
...
export async function closeFiscalYear(id: string) {
  ...
  if (fy.isClosed) throw new AppError('Fiscal year is already closed', 400);
