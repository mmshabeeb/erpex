...
export async function listProjects(filters: {
  status?: string; contactId?: string; search?: string; page?: number; pageSize?: number;
}) {
  const where: any = {};
...
export async function createProject(data: any) {
  ...
  return prisma.project.create({
    data: {
      ...
      tasks: tasks ? { create: tasks.map((t: any, i: number) => ({ ...t, sortOrder: t.sortOrder ?? i })) } : undefined,
    },
...
export async function listTimesheets(filters: {
  projectId?: string; userId?: string; approvalStatus?: string; page?: number; pageSize?: number;
...
export async function createTimesheet(data: any) {
  ...
  if (!hourlyRate && data.taskId) {
    ...
    if (task) hourlyRate = task.hourlyRate;
  }
