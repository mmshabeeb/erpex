...
import { NORMAL_BALANCE, AccountType } from '@erpex/shared';
import type {
  ITrialBalance, ITrialBalanceRow,
  IProfitLoss, IProfitLossSection, IProfitLossRow,
  IBalanceSheet, IBalanceSheetSection, IBalanceSheetRow,
} from '@erpex/shared';
...
export async function getTrialBalance(
  ...
  for (const account of accounts) {
    ...
    if (openingBalance === 0 && debitMovement === 0 && creditMovement === 0) continue;
    ...
    // Accumulate totals
    if (openingBalance >= 0) totalOpeningDebit += openingBalance;
    else totalOpeningCredit += Math.abs(openingBalance);

    totalDebitMovement += debitMovement;
    totalCreditMovement += creditMovement;

    if (closingBalance >= 0) totalClosingDebit += closingBalance;
    else totalClosingCredit += Math.abs(closingBalance);
  }
...
export async function getProfitLoss(
  ...
  async function getAccountBalances(type: string): Promise<IProfitLossRow[]> {
    const accounts = await prisma.account.findMany({
      where: { type: type as any, isActive: true },
      orderBy: { code: 'asc' },
    });

    const results: IProfitLossRow[] = [];
    for (const acct of accounts) {
      ...
      const netAmount = items.reduce((s, i) => {
        if (type === 'REVENUE') return s + Number(i.credit) - Number(i.debit);
  ...
  const cogsAccounts = expenseAccounts.filter(a =>
    a.accountName.toLowerCase().includes('cost of goods') ||
    a.accountName.toLowerCase().includes('cogs')
  );
  const otherExpenses = expenseAccounts.filter(a =>
    !cogsAccounts.some(c => c.accountId === a.accountId)
  );

  // Separate other income from main revenue
  const otherIncomeAccounts = revenueAccounts.filter(a =>
    a.accountName.toLowerCase().includes('other income') ||
    a.accountName.toLowerCase().includes('miscellaneous')
  );
  const mainRevenue = revenueAccounts.filter(a =>
    !otherIncomeAccounts.some(o => o.accountId === a.accountId)
  );
...
export async function getBalanceSheet(asOfDate: string): Promise<IBalanceSheet> {
  const endDate = new Date(asOfDate);

  async function getAccountBalances(type: string): Promise<IBalanceSheetRow[]> {
    const accounts = await prisma.account.findMany({
      where: { type: type as any, isActive: true },
      orderBy: { code: 'asc' },
    });

    const results: IBalanceSheetRow[] = [];
    for (const acct of accounts) {
      const items = await prisma.journalItem.findMany({
        where: {
          accountId: acct.id,
      ...
      if (type === 'LIABILITY' || type === 'EQUITY') balance = -balance;
